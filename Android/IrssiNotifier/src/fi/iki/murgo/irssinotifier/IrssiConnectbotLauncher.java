...
public class IrssiConnectbotLauncher {
    ...
    public static boolean launchIrssiConnectbot(Context context) {
        if (!IntentSniffer.isIntentAvailable(context,
                IrssiConnectbotLauncher.INTENT_IRSSICONNECTBOT))
            return false;

        Intent icbActivity = new Intent(context.getPackageManager().getLaunchIntentForPackage(
                INTENT_IRSSICONNECTBOT));
